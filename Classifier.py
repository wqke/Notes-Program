#Use scikit-learn package to train signal-background data, the output variable is called 'BDT'
...
#The training recipe : variables we use to distinguish two sets of data (here the choice is made for model independence)
...
data_sample_fit = data_sample_fit.sample(n=150000,random_state=int(400))
...
background_sample_fit['Dplus'] = background_sample_fit['Dplus'].sample(n=int(sample_length),random_state=int(sample_length/1000.))
...
#the classifier doesn't like 'NaN' values so we need to cast them (df=df.query...)
...
backgr = rec2array(backgr)     #we need to convert them to 2D array for training 


X = np.concatenate((signal, backgr))
y = np.concatenate((np.ones(signal.shape[0]),np.zeros(backgr.shape[0])))     #Assign features : signal=1 , bkg=0



X_dev,X_eval, y_dev,y_eval = train_test_split(X, y,test_size=0.33, random_state=42)
X_train,X_test, y_train,y_test = train_test_split(X_dev, y_dev,test_size=0.33, random_state=492)  #train on 67% and test on 33%
...
compare_train_test(bdt, X_train, y_train, X_test, y_test)     #we have histograms of bkg vs signal in train-test
...
array2root(y_predicted, "/home/ke/tmps/test-prediction.root", "DecayTree")

joblib.dump(bdt, 'bdt.joblib')    #save the training result to a joblib, we can apply later this file to predict other data 
                                  #and obtain their own BDT
